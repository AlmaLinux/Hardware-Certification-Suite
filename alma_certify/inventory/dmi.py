            ...
            for part in parts:
                ...
                if part.startswith("DMI type"):
                    try:
                        ...
                        pass
            # The handle as printed. Structures reference each other by it - a cache names the
            # processor it belongs to, a memory device names its array - so a reader joining them
            # needs it, and it costs one split to keep.
            handle = parts[0].split()[1].rstrip(",") if len(parts[0].split()) > 1 else None
            current = {"handle": handle, "dmi_type": dmi_type, "name": None, "props": {}}
