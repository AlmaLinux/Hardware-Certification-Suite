...
def build_summary(parsed: Dict[str, Any], redact: bool = False) -> Dict[str, Any]:
    ...
    summary = {
        ...
        "fields": _fields(dmi_records, parsed, redact),
    }
    return summary


# Structure names dmidecode prints, as keys a reader can address without knowing DMI type numbers.
# Anything not here keeps its type number, so nothing is dropped for want of a translation.
_DMI_KEYS = {
    0: "bios", 1: "system", 2: "baseboard", 3: "chassis", 4: "processor", 7: "cache",
    8: "port_connector", 9: "system_slot", 11: "oem_strings", 16: "physical_memory_array",
    17: "memory_device", 32: "system_boot", 38: "ipmi_device", 39: "system_power_supply",
    41: "onboard_device",
}

# Props that identify a machine rather than describe its parts. Dropped with ``--redact``, which
# already blanks the three the summary promotes; the same fields are on a dozen other structures
# and would otherwise walk straight back in through here.
_IDENTIFYING_PROPS = frozenset({
    "Serial Number", "UUID", "Asset Tag", "Asset Tag Number",
})


def _fields(dmi_records, parsed: Dict[str, Any], redact: bool) -> Dict[str, Any]:
    """Every field the source tools printed, as JSON, for a reader to compose an identity from.

    Not ``inventory.raw``, which is a map of tool names to the bundle paths of their untouched
    output. This is that same output parsed, so a rule can read a value without a reader shipping
    a parser for dmidecode.

    The summary above is a schema: named fields, one shape, a contract. This is the other half of
    "record what was measured and let the reader decide" - everything dmidecode and lspci printed,
    grouped by what it describes and otherwise untouched. A naming rule written a year from now
    can read a field nobody thought worth promoting, on bundles submitted today, without a new
    release of this suite.

    It is identity-bearing in a way the summary is not: serials and asset tags appear on most DMI
    structures, not only the three that are promoted. ``--redact`` drops them here too.
    """
    dmi: Dict[str, List[Dict[str, Any]]] = {}
    for record in dmi_records:
        dmi_type = record.get("dmi_type")
        key = _DMI_KEYS.get(dmi_type, "type_%s" % dmi_type)
        props = {
            name: value for name, value in (record.get("props") or {}).items()
            if not (redact and name in _IDENTIFYING_PROPS)
        }
        dmi.setdefault(key, []).append({
            # The handle joins a structure to the ones that reference it, which is how a cache or
            # a memory device is tied to the processor or array it belongs to.
    ...
    return {
        "dmi": dmi,
        # Verbatim, every key ``lspci -vmmnnk`` printed. ``pci_devices`` above keeps six of them in
        # a fixed shape because the server keys categorization on those; this keeps the rest.
        "pci": [dict(device) for device in parsed.get("pci", {}).get("devices", [])],
    }
